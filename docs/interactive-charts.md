...
6. **Watermarks on Non-public Charts:**
   - All non-public charts contain a watermark.
   - Frontend responsibility is limited to sending `is_public=false` and `data_classification` context (eg "Official-Sensitive") so backend can render the correct watermark text copy.
   - Chart watermark generation is backend-owned and is part of the canonical chart artifact returned by the chart API endpoint.

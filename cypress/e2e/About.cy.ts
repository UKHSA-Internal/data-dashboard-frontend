describe('About', () => {
  it('displays a title, body and related links', () => {
    ...
    cy.findByText(
      'The UKHSA dashboard provides a wide range of public health data in an easily accessible format. At the moment, the dashboard is focused on respiratory viruses.'
    )
    cy.findByText(
      'Data is updated in-line with the data source. It will be clearly signposted on each page the frequency of the upload and when the data was last updated.'
    )

    cy.findByRole('heading', { name: 'Respiratory viruses', level: 3 })
    cy.findByText(
    ...
    cy.findByRole('heading', { name: 'Viruses', level: 3 })
    ...
    cy.findByRole('heading', { name: 'Cases', level: 3 })
    cy.findByText(
    ...
    cy.findByRole('heading', { name: 'Hospitalisations', level: 3 })
    cy.findByText(
    ...
    cy.findByRole('heading', { name: 'Vaccines', level: 3 })

...
export async function processPreviewMode(request: NextRequest): Promise<NextResponse | undefined> {
  // PREVIEW ROUTES
...
export async function processStandardMode(request: NextRequest): Promise<NextResponse | undefined> {
...
export const middleware: NextMiddleware = async (request: NextRequest) => {
  let response = await processPreviewMode(request)
  if (response) {
    return response
  }

  response = await processStandardMode(request)
  if (response) {
    return response
  }
}

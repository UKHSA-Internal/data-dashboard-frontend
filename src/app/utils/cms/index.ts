...
export async function getPageMetadata(
  urlSlug: Slug,
  searchParams: SearchParams<{ page: number; search: string; areaName: string; areaType: string, categoryFilter: string; topicFilter: string }>,  
  pageType: PageType
): Promise<Metadata> {
  ...
  const topicFilter = searchParams.topicFilter
  ...
  try {
    ...
    if (pageType === PageType.MetricsParent) {
      const metricsEntries = await getMetricsPages({ search, page, categoryFilter, topicFilter })

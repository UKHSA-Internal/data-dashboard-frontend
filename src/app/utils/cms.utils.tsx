...
export const renderSection = (
  { id, value: { heading, content } }: z.infer<typeof Body>[number],
  ...
  <div key={id} className="govuk-!-margin-bottom-9" data-testid={`section-${kebabCase(heading)}`}>
    ...
    {content.map((card) => renderCard(card, searchParams))}
  </div>
)

export const renderCard = (
  { id, type, value }: z.infer<typeof ContentTypes>,
  searchParams: { [key: string]: string | string[] | undefined }
) => {
  return (
    <div key={id}>
                ...
                >
                  ...
                  {column.type === 'chart_with_headline_and_trend_card' && (
                    <>
                      ...
                      <Chart data={column.value} size={size} searchParams={searchParams} />

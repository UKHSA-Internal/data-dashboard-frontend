...
test('renders children within a list item', async () => {
  const { getAllByRole, queryByRole } = render(await ListItem({ children: 'List item content test' }))
...
test('shows horizontal rule', async () => {
  const { getByRole } = render(await ListItem({ children: 'List item content test', showRule: true }))

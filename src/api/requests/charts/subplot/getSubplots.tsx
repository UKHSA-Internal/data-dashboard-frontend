...
export const requestSchema = z.object({
  ...
  y_axis_minimum_value: z.number().nullable().optional(),pchart
...
export const getSubplots = async (chart: RequestParams) => {
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.code === 400) {
        logger.info('POST failed (no data) charts/subplot/v1 %s', subplots.map((plot) => plot.subplot_parameters.metric).join())
      } else {
